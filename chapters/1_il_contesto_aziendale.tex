...
\noindent
Un'aspetto molto importante del settore in cui opera \textit{Bluewind} è la presenza, per molti campi di utilizzo dei prodotti \textit{embedded}, di normative e \textit{standar} da rispettare per soddisfare i requisiti di sicurezza di alcuni settori critici.
...
\begin{itemize}
	...
	\item \textbf{Specialisti di \textit{Functionl Safety}} : data la rilevanza del tema della \textit{functional safety}, all'interno dell'azienda vi è un gruppo di addetti che si occupa esclusivamente di questa materia
...
Da quello che ho potuto cogliere, nell'organizzazione aziendale di \textit{Bluwind} la metodologia \textit{agile} non si limita a regolare il metodo di avanzamento dei progetti e delle attività dell' \textit{R\&D}, ma rappresenta anche un importante fondamento del loro modello economico.\\
