...
\noindent
\textit{GitLab} è una piattaforma che integra in un unico strumento la gestione del codice sorgente, il controllo di versione basato su \textit{Git}, e la gestione dei progetti. Permette inoltre di adottare l'integrazione e distribuzione continua, \gls{ci_cd}, ossia la possibilità di automatizzare il rilascio del \textit{software} e l'avvio di una sequenza di \textit{test}.
\noindent
...
\noindent
\textit{StrictoDoc} è uno strumento utilizzato per la scrittura di documentazione, gratuito e \gls{open_source_g}(ovvero il cui codice è pubblicamente disponibile e modificabile liberalmente dall'utente).
...
È uno strumento su cui \textit{Bluewind} sta puntando molto per via della sua semplicità di utilizzo e della possibilità, in quanto \textit{open source}, di aggiunge funzionalità \textit{custom} in base alle proprie esigenze.
