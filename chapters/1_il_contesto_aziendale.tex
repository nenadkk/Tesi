...
\fi
\noindent
Fondata nel 1998, \textit{Bluewind} è un'azienda informatica specializzata nel campo dei sistemi \gls{embedded_g} e della sicurezza informatica.
...
\noindent
Un'aspetto molto importante del settore in cui opera \textit{Bluewind} è la presenza, per molti campi di utilizzo dei prodotti \gls{embedded_g}, di normative e \textit{standar} da rispettare per soddisfare i requisiti di sicurezza di alcuni settori critici.
Per questo motivo \textit{Bluewind} ha raccolto al suo interno una significativa conoscenza sulla \gls{func_saf_g}, arrivando ad offrire servizi di consulenza e di validazione su questo tema ad aziende terze, e riuscendo a fare di questo ambito uno dei suoi principali punti di forza e competitività.\\
...
Recentemente Blewind ha accolto all'interno del suo \textit{team} alcuni dipendenti che hanno già lavorato in passato come enti certificatori, in modo da poter rafforzare le proprie competenze nella materia.\\

\noindent
Ad oggi, \textit{Bluewind} è presente sul territorio con due sedi: una sede principale, a Castelfranco Veneto (VI), che si occupa delle attività di ricerca, sviluppo e \textit{marketing}, e una sede secondaria a Milano focalizzata sulla \gls{func_saf_g} e sulla sicurezza informatica in generale.
...
Durante la mia attività di tirocinio sono stato inserito all'interno del reparto \gls{ReD} e ho avuto modo di conoscere i principi alla base del loro metodo di lavoro.\\\\
\noindent
...
\begin{itemize}
	...
	\item \textbf{Specialisti di \textit{Functionl Safety}} : data la rilevanza del tema della \gls{func_saf_g}, all'interno dell'azienda vi è un gruppo di addetti che si occupa esclusivamente di questa materia
...
Da quello che ho potuto cogliere, nell'organizzazione aziendale di \textit{Bluwind} la metodologia \gls{agile_g} non si limita a regolare il metodo di avanzamento dei progetti e delle attività dell' \gls{ReD}, ma rappresenta anche un importante fondamento del loro modello economico.\\
...
I progetti sono assegnati a \textit{team} di circa 3 persone (o più, in base al carico di lavoro), tra sviluppatori e analisti, con alla direzione un \gls{po} che volge il ruolo di \textit{scrum master}. Quest'ultimo si occupa quindi di agevolare la corretta applicazione del \textit{framework} \gls{scrum_g}.\\
...
Coinvolgere il cliente ai vari stadi del progetto e motivarne la partecipazione, con una frequenza di riunioni che va oltre quella di un approccio \gls{Agile} classico, è uno dei principali impegni di \textit{Bluewind}, che ha saputo dimostrare la sua efficacia.\\
...
\fi
\noindent
Occupandosi di sistemi \gls{embedded_g}, i domini applicativi dei prodotti e dei servizi di \textit{Bluewind} spaziano in molteplici mercati.
...
L'azienda che riceve l'incarico dall'\gls{OEM} prende il nome di azienda \textit{tier 1}.
...
Questo cambia il tipo di relazione che \textit{Bluewind} ha con l'azienda committente e il progetto; nel caso di lavori svolti per delle \gls{OEM}, \textit{Bluewind} non ha modo di vedere come si integra il suo operato all'interno del progetto principale, in quanto la sua visione rimane limitata al singolo elemento che le è stato affidato.
...
\noindent
Il linguaggio \textit{C} rimane il linguaggio dominante nel settore \gls{embedded_g}, in quanto offre un controllo quasi diretto sull'\textit{hardware} (registri, memoria, interrupt) mantenendo comunque un livello di astrazione superiore all'\textit{Assembly}.
...
L'\textit{Assembly} resta il linguaggio più basso disponibile nell'\gls{embedded_g}, avendo una corrispondenza vicina all'uno a uno con le istruzioni macchina dell'architettura.
Quindi viene utilizato quando serve controllo assoluto sull'\textit{hardware}, accesso a istruzioni specifiche non esposte dal \textit{C} o un'ottimizzazione estrema di alcune funzioni critiche.


\subsection{Strumenti proprietari di aziende \textit{partner} come \textit{Infineon} e \textit{ST}}
\noindent
Nell'ambito \gls{embedded_g} è spesso richiesto l'utilizzo di componenti \textit{hardware} costruite su misura per il loro caso di applicazione.
...
Questo strumento si integra molto bene con il metodo \gls{Agile} adottato dall'azienda, permettendo di organizzare, suddividere e tenere traccia delle attività in maniera più organizzata e meticolosa.
...
È uno strumento su cui \textit{Bluewind} sta puntando molto per via della sua semplicità di utilizzo e della possibilità, in quanto \gls{open_source_g} di aggiunge funzionalità \textit{custom} in base alla propria esigenza.
...
\noindent
\textit{PC-lint} è uno strumento di analisi statica per \textit{C} e \textit{C++} utilizzato in ambienti \textit{safety-critical} per scoprire \textit{bug}, vulnerabilità e verificare il rispetto delle regole imposte dagli \textit{standard}, tra cui anche le regole \gls{misra}.\\
